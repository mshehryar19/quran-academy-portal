...
@section('content')
    <div class="mb-6">
        <h2 class="text-xl font-semibold">{{ $student->full_name }} — {{ $invoice->invoice_number }}</h2>
        <p class="text-sm text-gray-600"><a href="{{ route('parent.billing.student.invoices', $student) }}" class="underline">Back to {{ $student->full_name }}&apos;s invoices</a></p>
    </div>
    ...
    <section class="rounded-lg border border-gray-200 bg-white p-6 text-sm shadow-sm">
        ...
        <ul class="mt-2 space-y-2">
            @forelse ($invoice->payments as $p)
                <li>{{ $p->paid_on->format('Y-m-d') }} — {{ number_format($p->amount, 2) }} {{ $p->currency }}</li>
            @empty
