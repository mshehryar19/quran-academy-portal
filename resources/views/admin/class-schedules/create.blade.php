...
@section('content')
    <div class="mb-6">
        <h2 class="text-xl font-semibold">Add class schedule</h2>
        <p class="text-sm text-gray-600">Creates a recurring weekly class. Conflicts with other active schedules are blocked.</p>
    </div>

    <form method="post" action="{{ route('admin.class-schedules.store') }}" class="max-w-2xl rounded-lg border border-gray-200 bg-white p-6 shadow-sm">
        @csrf
        <div class="grid grid-cols-1 gap-4 md:grid-cols-2">
            <div class="md:col-span-2">
                ...
                <select id="teacher_id" name="teacher_id" required class="mt-1 w-full rounded-md border border-gray-300 px-3 py-2 text-sm shadow-sm focus:border-gray-900 focus:outline-none">
                    ...
                    @foreach ($teachers as $t)
                        <option value="{{ $t->id }}" @selected(old('teacher_id') == $t->id)>{{ $t->public_id }} — {{ $t->full_name }}</option>
                    @endforeach
                </select>
            ...
            <div class="md:col-span-2">
                ...
                <select id="student_id" name="student_id" required class="mt-1 w-full rounded-md border border-gray-300 px-3 py-2 text-sm shadow-sm focus:border-gray-900 focus:outline-none">
                    ...
                    @foreach ($students as $s)
                        <option value="{{ $s->id }}" @selected(old('student_id') == $s->id)>{{ $s->public_id }} — {{ $s->full_name }}</option>
                    @endforeach
                </select>
            ...
            <div class="md:col-span-2">
                ...
                <select id="class_slot_id" name="class_slot_id" required class="mt-1 w-full rounded-md border border-gray-300 px-3 py-2 text-sm shadow-sm focus:border-gray-900 focus:outline-none">
                    <option value="">— Select —</option>
                    @foreach ($slots as $sl)
                        <option value="{{ $sl->id }}" @selected(old('class_slot_id') == $sl->id)>{{ $sl->name ? $sl->name.' — ' : '' }}{{ $sl->timeRangeLabel() }}</option>
                    @endforeach
                </select>
            ...
            <div>
                ...
                <select id="day_of_week" name="day_of_week" required class="mt-1 w-full rounded-md border border-gray-300 px-3 py-2 text-sm shadow-sm focus:border-gray-900 focus:outline-none">
                    @include('admin.partials.day-of-week-options', ['selected' => old('day_of_week', 1)])
                </select>
        ...
        <div class="mt-6 flex gap-3">
            <button type="submit" class="rounded-md bg-gray-900 px-4 py-2 text-sm font-medium text-white hover:bg-gray-800">Create schedule</button>
            <a href="{{ route('admin.class-schedules.index') }}" class="rounded-md border border-gray-300 px-4 py-2 text-sm text-gray-700 hover:bg-gray-50">Cancel</a>
        </div>
    </form>
@endsection
